...
public class PedidoCriadoConsumerTests
{
    ...
    [Fact]
    public async Task Consumir_ComMultiplosProdutos_DeveReservarTodosEPublicarSucesso()
    {
        ...
        var evento = new PedidoCriadoEvento(
            pedidoId,
            Guid.NewGuid(),
            new List<ItemPedidoEvento>
            {
                new(produto1Id, 1, 50m),
                new(produto2Id, 2, 30m)
            },
            110m,
            DateTime.UtcNow);

        _produtoServiceMock.Setup(s => s.GetByIdAsync(produto1Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ProdutoDto(produto1Id, "Produto 1", null, 50m, null,
                ProdutoStatus.Disponivel, Guid.NewGuid(), "Cat", DateTime.UtcNow, DateTime.UtcNow));
        _produtoServiceMock.Setup(s => s.GetByIdAsync(produto2Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ProdutoDto(produto2Id, "Produto 2", null, 30m, null,
                ProdutoStatus.Disponivel, Guid.NewGuid(), "Cat", DateTime.UtcNow, DateTime.UtcNow));
        _produtoServiceMock.Setup(s => s.ReservarAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        _busMock.Setup(b => b.Publish(It.IsAny<ProdutosReservadosEvento>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        _produtoServiceMock.Verify(s => s.ReservarAsync(produto1Id, It.IsAny<CancellationToken>()), Times.Once);
        _produtoServiceMock.Verify(s => s.ReservarAsync(produto2Id, It.IsAny<CancellationToken>()), Times.Once);
        _busMock.Verify(b => b.Publish(
            It.Is<ProdutosReservadosEvento>(e => e.Sucesso && e.ProdutosReservados.Count == 2),
            It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task Consumir_QuandoSegundoProdutoNaoEncontrado_NaoDeveReservarPrimeiro()
    {
        var produto1Id = Guid.NewGuid();
        var produto2IdInexistente = Guid.NewGuid();
        var pedidoId = Guid.NewGuid();
        var evento = new PedidoCriadoEvento(
            pedidoId,
            Guid.NewGuid(),
            ...
            {
                new(produto1Id, 1, 50m),
                new(produto2IdInexistente, 1, 30m)
            },
            80m,
            DateTime.UtcNow);
        ...
        _produtoServiceMock.Setup(s => s.GetByIdAsync(produto1Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new ProdutoDto(produto1Id, "Produto 1", null, 50m, null,
                ProdutoStatus.Disponivel, Guid.NewGuid(), "Cat", DateTime.UtcNow, DateTime.UtcNow));
        _produtoServiceMock.Setup(s => s.GetByIdAsync(produto2IdInexistente, It.IsAny<CancellationToken>()))
            .ReturnsAsync((ProdutoDto?)null);
        _produtoServiceMock.Setup(s => s.ReservarAsync(produto1Id, It.IsAny<CancellationToken>()))
            .ReturnsAsync(true);
        _busMock.Setup(b => b.Publish(It.IsAny<ProdutosReservadosEvento>(), It.IsAny<CancellationToken>()))
            .Returns(Task.CompletedTask);
        ...
        _busMock.Verify(b => b.Publish(
            It.Is<ProdutosReservadosEvento>(e => !e.Sucesso),
            It.IsAny<CancellationToken>()), Times.Once);
    }
}
